...
import FilterProduct, { FilterProductValue } from '../../components/FilterProduct';
import axios from 'axios';
import { ProductModel, SubProductModel } from '../../models/Product';
import AddSubProductModal from '../../modals/AddSubProductModal';
...
const Inventories = () => {
	...
	// Fix lỗi bị trùng cái size (Ví dụ: Một sản phẩm chỉ có 1 size XL) do dummyData tạo ra
	const hadnleFixDemoData = async () => {
		// console.log(products.length)
		products.forEach(async (item, index) => {
			// const cats = item.categories[0]	?? '66fd64d9909d3dbc02cdb0cb';
			// console.log(cats);
			const subs = item.subItems;
			const items: string[] = [];

			subs.forEach(async (sub) => {
				if (items.includes(sub.size)) {
					const id = sub._id
					try {
						// console.log(`/products/remove-sub-product?id=${id}`)
						await handleAPI(`/products/remove-sub-product?id=${id}`, undefined, 'delete');
						console.log('delete!!!')

						index === products.length - 1 && console.log('Done')
					} catch (error) {
						console.log(error);
					}
				} else {
	...
	return (
		<div>
			{/* <Button onClick={hadnleFixDemoData}>Add Demo Product</Button> */}
			<div className='row'>
				<div className='col'>
